...
import com.neusoft.emr.common.config.Global;
...
public class JedisUtils {
	...
	public static long hset(String key,String field,Object value){
		...
		try {
			...
			result = jedis.hset(getBytesKey(key), getBytesKey(field), ObjectUtils.serialize(value));

			logger.debug("setObject {},{} = {}", key,field, value);
		} catch (Exception e) {
			logger.warn("setObject {} ,{}= {}", key,field, value, e);
		} finally {
			returnResource(jedis);
		}
		return result;
		...
		try {
			...
			if (jedis.exists(getBytesKey(key))) {
				if (jedis.hexists(getBytesKey(key),getBytesKey(field))){
					...
					logger.debug("delObject {},{}", key,field);
				}else{
					logger.debug("delObject {},{} not exists", key,field);
				}
			}

		} catch (Exception e) {
			logger.warn("delObject {},{}", key,field, e);
		} finally {
			returnResource(jedis);
		}
		return result;
	...
	public static boolean hexists(String key,String field){
		...
		try {
			...
			logger.debug("existsObject {},{}", key,field);
		} catch (Exception e) {
			logger.warn("existsObject {},{}", key,field, e);
		} finally {
			returnResource(jedis);
		}
		return result;
